...
class MultiPlatformImageDownloader(ctk.CTk):
    def __init__(self):
        ...
        self.size_presets = {
            ...
            "甘肃尺寸": {"vertical": (375, 562), "horizontal": (562, 375)},
...
#        self.delete_status_label.pack(side="left", padx=10, pady=10)
    ...
    def download_selected(self):
        ...
        for i, (title, img_url, img_type, width, height) in enumerate(items_to_process):
            try:
                ...
                # 下载图片 - 使用设置中的filename_format，不传递suffix
                success = self.download_image(
                    img_url, title, download_path, 
                    suffix=None,  # 不传递suffix，让方法使用filename_format
                    size_str=size_str,
                    platform=platform,
            ...
            if suffix:
                ...
                try:
                    base_filename = self.filename_format.format(
                        **{k: format_data.get(k, '') for k in ['标题', '图片尺寸', '类型', '平台']}
                    )
    ...
    def open_settings_window(self):
        ...
        explanation = (
            "可用占位符:\n"
            "  {标题}: 原始标题 (已清理特殊字符)\n"
            "  {图片尺寸}: 图片的目标尺寸 (例如 '412x600' 或 '原尺寸')\n"
            "  {类型}: 图片类型 ('竖图' 或 '横图', 仅当下载类型为'全部'且非腾讯时有效)\n"
            "  {平台}: 视频平台 ('爱奇艺', '腾讯视频', '优酷视频')\n"
            "示例: {标题}_{类型}_{图片尺寸} -> 示例标题_竖图_412x600.jpg\n"
            "示例: {标题}_{类型}_{图片尺寸}_{平台} -> 示例标题_竖图_412x600_爱奇艺.jpg"
        )
