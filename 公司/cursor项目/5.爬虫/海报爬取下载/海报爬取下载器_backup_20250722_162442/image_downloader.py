...
#        self.delete_status_label.pack(side="left", padx=10, pady=10)
                ...
                if result_text == "未找到结果":
                    ...
                    if lines and " - " in lines[0]:
                        ...
                        movie_label = ctk.CTkLabel(movie_cell, text=movie_name, 
                                                font=ctk.CTkFont(size=11), wraplength=100)
                        movie_label.grid(row=0, column=0, padx=6, pady=4, sticky="w")
        ...
        for data_row in processed_data:
            ...
            for col, platform in enumerate(platforms):
                ...
                movie_label = ctk.CTkLabel(row_frame, text=movie_text, 
                                         wraplength=120, font=ctk.CTkFont(size=12))
    ...
    def download_selected(self):
        ...
        for i, var in enumerate(self.result_vars):
            if var.get():
                if i < len(self.result_checkboxes):
                    title = self.result_checkboxes[i].cget("text")
                    selected_titles.append(title)
                    selected_platforms[title] = []
        
        # 获取选中的平台
        for platform, platform_vars in self.platform_vars.items():
            for i, var in enumerate(platform_vars):
                if var.get():
                    # 找到对应的标题
                    if i < len(selected_titles):
                        title = selected_titles[i]
                        if title in selected_platforms:
                            selected_platforms[title].append(platform)
        ...
        for title, platforms in selected_platforms.items():
            if not platforms:  # 如果标题被选中但没有选择平台，下载所有平台
                platforms = ["爱奇艺", "腾讯视频", "优酷视频"]
            
            for platform in platforms:
                # 查找该平台的结果
                platform_result = None
                for result in self.results[platform]:
                    if len(result) >= 2 and result[0] == title:
                        platform_result = result
                        break
                
                if platform_result and len(platform_result) >= 2:
                    img_url = platform_result[1]
                    
                    # 根据方向确定下载任务
                    if orientation == "全部":
                        v_w, v_h = target_vertical_size
                        items_to_process.append((title, img_url, "竖图", v_w, v_h, platform))
                        
                        if platform != "腾讯视频":  # 腾讯视频不支持横图
                            h_w, h_h = target_horizontal_size
                            items_to_process.append((title, img_url, "横图", h_w, h_h, platform))
        ...
        for i, (title, img_url, img_type, width, height, platform) in enumerate(items_to_process):
            try:
                # 构建文件名
    ...
    def download_image(self, base_img_url, title, download_path, suffix=None, size_str=None,
        ...
        try:
            ...
            target_filesize_kb = None
            if preset_name == "云南尺寸":
                target_filesize_kb = 100
            elif preset_name == "原尺寸":
                target_filesize_kb = 500
            if target_filesize_kb is not None:
                ...
                if current_size_kb > target_filesize_kb:
                    try:
                        ...
                        return False, f"压缩失败:{compress_err}"
            ext = ".jpg"
            if use_cid_filename:
    ...
    def batch_crawling_worker(self):
        ...
        try:
            ...
            for i in range(self.current_batch_row, total_rows):
                ...
                result = self.search_with_priority(movie_name)
                if result:
                    success = self.batch_download_images(result, cid, movie_name)
                    if success:
    ...
    def batch_download_images(self, result, cid, movie_name):
