...
xlsx = "D:\\物料转接\\上传\\1.本地表格模板.xlsx"
out_xlsx = "D:\\物料转接\\上传\\out.xlsx"
test_xlsx = "D:\\物料转接\\上传\\3.上传表格模板.xlsx"
summary_xlsx = "D:\\物料转接\\上传\\2.新片汇总.xlsx"
...
max_row = 2
Header = {1: '序号', 2: '节目ID', 3: '节目名称', 4: '节目类型', 5: '内容类型', 6: '出品地区', 7: '出品年代', 8: '导演', 9: '主演', 10: '集数', 11: '单集时长', 12: '上线时间', 13: '发行许可证号', 14: '院线上映时间', 15: '免费播出时间', 16: '授权开始时间', 17: '授权结束时间', 18: '是否独家', 19: '表格更新日期', 20: '版权范围'}
...
if os.path.exists(summary_xlsx):
    ...
    sheet_summary = workbook_summary.active
    # 如果是新创建的文件，添加表头
    for col_num, header_text in Header.items():
        cell = sheet_summary.cell(row=1, column=col_num, value=header_text)
        cell.font = font
...
for row in row_numbers:
    ...
    while i <= time:
        ...
        # 遍历所有列，确保每个单元格都有正确的样式
        for col_num in range(1, len(Header) + 1):
            cell = sheet_summary.cell(row=summary_row, column=col_num)
            
            # 设置单元格值
            if col_num == 1:  # 序号列
                cell.value = summary_row + 1
            elif col_num == 19:  # 表格更新日期列
                cell.value = current_date
            elif col_num == 20:  # 版权范围列
                cell.value = 'iptv+ott'
            elif col_num in final_dict:  # 其他列从final_dict中获取值
                value = final_dict[col_num]
                if col_num in time_fields:
                    value = format_time(value)
                elif col_num == exclusive_field:
                    value = format_exclusive_status(value)
