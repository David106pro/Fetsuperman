...
#   函数调用
def use_Function(movie_response, row_index):
    ...
    src = get_post(soup)
    file_size_mb, save_state = save_post(src, movie_name)
    ...
    print(f"海报src：{src}   海报大小：{file_size_mb}   海报存取状态：{save_state}")
    return name, year, place, typ, director, actors, count, time, language, sb_name, rating, votes, src, file_size_mb, save_state

href = get_href(search_url)
movie_response = requests.get(href, headers=headers)
if movie_response.status_code == 200:
    result = use_Function(movie_response,1)
else:
