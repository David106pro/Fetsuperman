...
# 获取评分+评论人数
def get_rating(soup):
    temple = soup.find('strong', {'class': 'll rating_num', 'property': 'v:average'})
    if temple:
        rating = temple.text
...
def get_introduce(soup):
    temple = soup.find('span', property='v:summary')
    introduce = temple.text if temple else 'NO FOUND'
    return introduce
#   函数调用
def use_Function(movie_response, row_index, href):
    ...
    print(f"简介：{introduce}")
    return name, sb_name, year, place, typ, director, actors, count, time, language, rating, votes, introduce
#   表格存储
def save_to_excel(result, xlsx, row_index):
    for i,column in enumerate(range(4, 16)):
        if result[i] != "NO FOUND":
            sheet.cell(row=row_index, column=column, value=result[i])
    print("表格存储完毕")

# 加载模板文件的工作簿
xlsx = "C:\\Users\\fucha\\Desktop\\豆瓣通用模板.xlsx"
workbook = load_workbook(xlsx)
sheet = workbook.active

#——————主函数——————#
for row_index in range(2, sheet.max_row + 1):  # 从第二行开始，跳过标题行
    movie_name = sheet.cell(row=row_index, column=3).value  # 片名
    movie_state = sheet.cell(row=row_index, column=17).value  # 爬取状态
    if movie_state != "爬取成功":
        ...
        if href:
            ...
            result = use_Function(movie_response, row_index, href)
            save_to_excel(result, sheet, row_index)
            sheet.cell(row=row_index, column=17, value="爬取成功")
        else:
            sheet.cell(row=row_index, column=17, value="爬取失败")
    workbook.save(xlsx)