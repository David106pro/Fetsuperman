...
def crawl_movie_info(soup, n):
    ...
    id = maoyan_id(soup)
    return movie_name, score, rating_count, show_time, show_place, duration, year, actors, save_state, file_size_mb, src, id
...
with sync_playwright() as playwright:
    for i, name in enumerate(movie_names, 1):
        picture_name = name
        html = run(name, playwright)
        soup = BeautifulSoup(html, 'html.parser')
        result = crawl_movie_info(soup, i)
        print(result)
        print()  # 换行，使每部电影的输出结果更清晰分开